...
class MaterialApproval extends StatefulWidget {
  MaterialApproval({
    Key? key,
  }) : super(key: key);

  @override
  State<MaterialApproval> createState() => _MaterialApprovalState();
}

class _MaterialApprovalState extends State<MaterialApproval> {
  String netIm = "";
  String search = "";
  ...
  Column matList(list) {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: l);
  }
              ...
              children: [
                  ...
                  snapshot.data!.docs.map((doc) {
                    if (state == "all" ||
                      ...
                      return Card(
                        color: index.isEven
                            ? Color.fromARGB(255, 110, 42, 42)
                            : Color.fromARGB(255, 49, 36, 109),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text("Request Date : " +
                                  doc.get("requestDate").toDate().toString()),
                              if (doc.get("acceptDate") != null &&
                                  (doc.get("state") == "accepted" ||
                                      doc.get("state") == "delievered" ||
                                      doc.get("state") == "restored"))
                                Text("Accept Date : " +
                                    doc.get("acceptDate").toDate().toString()),
                              if (doc.get("takeDate") != null &&
                                  (doc.get("state") == "delievered" ||
                                      doc.get("state") == "restored"))
                                Text("Deliever Date : " +
                                    doc.get("takeDate").toDate().toString()),
                              if (doc.get("backDate") != null &&
                                  (doc.get("state") == "restored"))
                                Text("Restore Date : " +
                                    doc.get("backDate").toDate().toString()),
